...
class InstructorLessonControllerE2ETest {
    ...
    @Test
    void testGetAllLessons_Forbidden() {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(jwtToken);
        HttpEntity<Void> requestEntity = new HttpEntity<>(headers);

        ResponseEntity<String> responseEntityWithLessons = restTemplate.exchange(
        ...
        assertEquals(HttpStatus.FORBIDDEN, responseEntityWithLessons.getStatusCode());
    }
